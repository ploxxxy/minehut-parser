new gridjs.Grid({
  ...
  columns: [
    {
      ...
      data: (row) => row.name,
      formatter: (cell) => {
        document.title = `${cell} - Minehut server lookup`
        return cell
      },
    },
    {
    ...
    },
    {
      ...
      data: (row) =>
        row.online
          ? `Yes (${row.playerCount} players)`
          : new Date(row.last_online).toLocaleDateString(),
    },
    {
      name: 'Server Type',
  ...
  server: {
    url:
      'https://api.minehut.com/server/' + window.location.search.substring(3),
